...
// 1. Verificar si el ID está presente en la URL y si es un número entero válido.
if (!isset($_GET['id']) || !filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    // Si no hay ID o no es un número, redireccionar a la lista de rutinas.
    header('Location: Lista_Rutinas.html');
    exit();
}

// 2. ID dinámico obtenido de la URL
...
if (!$rutina) {
    // Si la rutina no existe en la DB
...
<body class="bg-background-light dark:bg-background-dark font-display text-gray-800 dark:text-gray-200">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // El ID se pasa de PHP a JS a través del atributo data-rutina-id
            const rutinaId = tableBody ? tableBody.dataset.rutinaId : null;

            // Si la rutina no fue encontrada, detenemos la ejecución de JS para evitar errores.
            if (!rutinaId) return;
            ...
            async function loadAvailableExercises() {
                ...
                try {
                    // Se envía el id_rutina como query param para excluir ejercicios ya añadidos.
            ...
            // Delegación de eventos para botones dinámicos (Agregar/Eliminar)
            document.addEventListener('click', async function(e) {

                // 1. Manejar botones Agregar del modal
                if (e.target.classList.contains('add-btn')) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            // Agregar a la tabla, remover del modal y actualizar músculos
                    ...
                    if (confirm(`¿Eliminar "${ejercicioName}" de la rutina?`)) {
                        try {
                            ...
                            if (result.success) {
                                row.remove();
                                updateMuscleTags();
                                // Opcional: Si la tabla queda vacía, re-insertar el mensaje de "No hay ejercicios"
