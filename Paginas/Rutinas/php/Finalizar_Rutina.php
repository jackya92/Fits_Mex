...
$ejercicios = isset($_GET['ejercicios']) ? intval($_GET['ejercicios']) : 0;

// Si aún no tienes un sistema de login funcionando al 100%, fuerza el usuario 1 temporalmente para que guarde los datos:
$user_id = $_SESSION['user_id'] ?? 1; 

if ($user_id > 0 && $id_rutina > 0) {
    try {
        $stmt = $pdo->prepare("SELECT nom_rutina FROM rutina WHERE id_rutina = ?");
        $stmt->execute([$id_rutina]);
        $nombre = $stmt->fetchColumn() ?: "Rutina";

        $sql = "INSERT INTO rutinas_realizadas (id_usuario, nom_rutina, fecha_completada, tiempo_total, ejercicios_completados) 
                VALUES (?, ?, CURDATE(), ?, ?)";
        $pdo->prepare($sql)->execute([$user_id, $nombre, $tiempo, $ejercicios]);
        
    } catch (Exception $e) { 
        // ¡No dejes esto vacío! Si hay un error, imprímelo para saber qué tabla o campo falta en MySQL
    ...
    <div class="flex">
        ...
        <div class="flex-1 flex flex-col min-h-screen md:ml-64">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="rounded-xl bg-white dark:bg-black/10 p-6 shadow-lg border border-primary/20">
                            ...
                            <p class="text-3xl font-bold text-gray-900 dark:text-white">
                                <?php echo htmlspecialchars($tiempo); ?>
                            </p>
                        </div>
                        ...
                        <div class="rounded-xl bg-white dark:bg-black/10 p-6 shadow-lg border border-primary/20">
                            ...
                            <p class="text-3xl font-bold text-gray-900 dark:text-white">
                                <?php echo htmlspecialchars($ejercicios); ?>
                            </p>
                        </div>
                    </div>
            ...
            <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-black/20 border-t border-primary/20 dark:border-primary/30 flex justify-around py-2 z-50">
                <a class="flex flex-col items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-primary" href="../../Paginas_principales/Pag_Principal.html">
                    <span class="material-symbols-outlined">home</span>
                    <span class="text-xs font-medium">Inicio</span>
                </a>
                <a class="flex flex-col items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-primary" href="../../Paginas/Ejercicios/php/ejercicios.php">
                    <span class="material-symbols-outlined">search</span>
                    <span class="text-xs font-medium">Explorar</span>
                </a>
                <a class="flex flex-col items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-primary" href="../Lista_Rutinas.html">
                    <span class="material-symbols-outlined">library_books</span>
                    <span class="text-xs font-medium">Mis rutinas</span>
                </a>
                <a class="flex flex-col items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-primary" href="../../Paginas/Perfil/Mi_Perfil.html">
                    <span class="material-symbols-outlined">account_circle</span>
